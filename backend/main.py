...
from ai_service import get_ai_response, create_negotiation_prompt, get_evaluation, extract_negotiation_state
...
def list_all_sessions_db():
    with get_db() as conn:
        rows = conn.execute(
            "SELECT session_id, created_at, data FROM sessions ORDER BY created_at DESC"
        ).fetchall()
    result = []
    for row in rows:
        data = json.loads(row[2])
        agreed = data.get('agreed_terms', {})
        result.append({
            "session_id": row[0],
            "created_at": row[1],
            "deal_closed": agreed.get('price') is not None,
            "final_price": agreed.get('price'),
            "final_delivery": agreed.get('delivery'),
        })
    return result
...
def list_all_sessions():
    return {"sessions": list_all_sessions_db()}
